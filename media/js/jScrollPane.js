	...
	b.fn.jScrollPane = function(f) {
		function d(C, L) {
			...
			function aa(aF) {
				...
				u(aI, aG);
				V.css("top", s);
				$('#firstcol table').css('margin-top', s);
			...
			function ab(aF) {
				...
				r(aI, aH);
				V.css("left", s);
				$('#table_header').css('margin-left', s);		
			...
			function I() {
				V.unbind("focus.jsp").bind("focus.jsp",
				function(s) {
					if (s.target === V[0]) {
						return
					}
			...
			function P() {
				var aF, s;
				C.attr("tabindex", 0).unbind("keydown.jsp").bind("keydown.jsp",
				function(aJ) {
					if (aJ.target !== C[0]) {
						return
					}
